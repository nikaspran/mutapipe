...
describe('mp()', function () {
  describe('pipe()', function () {
    ...
    it('should support built arguments using a custom build function', function (done) {
      var received = '';

      mp().pipe(es.merge,
        {build: mp().pipe(es.readArray, ['1'])}
      )().on('data', function (data) {
        received += data;
      }).on('end', function () {
        expect(received).to.contain('1');
  ...
  describe('previous()', function () {
    function storeData() {
      var stored = '';
      var handler = es.map(function (data, cb) {
        stored = data;
        cb(null, data);
      });
      handler._stored = function (arg) {
        return es.map(function (data, cb) {
          cb(null, arg ? stored + arg : stored);
        });
      };
      return handler;
    }

    it('should let you pipe into previous step results', function (done) {
      var pipeline = mp()
        .pipe(append, '1')
        .pipe(storeData)
        .pipe(append, '2')
        .pipe(mp.previous(by.task(storeData), '_stored'));
    ...
    it('should support arguments', function (done) {
      var pipeline = mp()
        ...
        .pipe(storeData)
        .pipe(append, '2')
        .pipe(mp.previous(by.task(storeData), '_stored'), 'Arg');

      write('abc').into(pipeline()).andExpect('abc1Arg').then(done);
    });

    it('should take the most recent step to restore', function (done) {
      var pipeline = mp()
        .pipe(append, '1')
        .pipe(storeData)
        .pipe(append, '2')
        .pipe(mp.previous(by.task(storeData), '_stored'), 'Earlier')
        .pipe(storeData)
        .pipe(append, '3')
        .pipe(mp.previous(by.task(storeData), '_stored'), 'Latest');

      write('abc').into(pipeline()).andExpect('abc1EarlierLatest').then(done);
    });
  });
