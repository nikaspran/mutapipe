...
describe('mutapipe()', function () {
  describe('pipe()', function () {
    it('should handle simple streams', function (done) {
      write('abc').into(mutapipe()()).andExpect('abc').then(done);
    });
    ...
    it('should allow you to supply your own step id', function () {
      ...
      var steps = pipeline.get(mutapipe.by.id('someId'));
      expect(steps[0]).to.eql({id: 'someId', task: append, args: ['1'], buildArgs: undefined});
      expect(steps.length).to.eql(1);
    });
    ...
    it('should support readable streams', function (done) {
      var received;
      mutapipe().pipe(es.readArray, ['abc'])()
    ...
    it('should support built arguments', function (done) {
      ...
      mutapipe().pipe({
        task: es.merge, buildArgs: [
          mutapipe().pipe(es.readArray, ['1']),
          mutapipe().pipe(es.readArray, ['2'])
        ]
      })
      ().on('data', function (data) {
        received += data;
  ...
  describe('get()', function () {
    ...
    it('should return all steps that match the predicate function', function () {
      ...
      expect(pipeline.get(function (step) {
        return step.task === ctor1;
      })).to.eql([{id: 0, task: ctor1, args: [], buildArgs: undefined}]);
    });
  });
  ...
  describe('insert()', function () {
    it('should be able to insert a task by constructor', function (done) {
      ...
      pipeline = pipeline.insert(append, '2').after(pipeline.get(by.task(append)));
    ...
    it('should be able to replace with another mutapipe pipeline', function (done) {
      ...
      pipeline = pipeline.insert(mutapipe().pipe(append, '2').pipe(append, '3')).after(pipeline.get(by.task(append)));
    ...
    it('should be able to replace with a step object', function (done) {
      ...
      pipeline = pipeline.insert({task: append, args: ['2']}).after(pipeline.get(by.task(append)));

      write('abc').into(pipeline()).andExpect('abc1234').then(done);
    });
    it('should leave the old pipeline intact', function (done) {
      var original = mutapipe().pipe(append, '1').pipe(append, '3').pipe(append, '4');
      var replaced = original.insert(append, '2').after(original.get(by.task(append)));
    ...
    it('should support nesting', function (done) {
      ...
      pipeline = pipeline.insert(
        mutapipe()
          ...
          .pipe(mutapipe()
            .pipe(append, 'N2'))
      ).after(pipeline.get(by.args('After')));
  ...
  describe('without()', function () {
    ...
    it('should be immutable', function (done) {
      var original = mutapipe().pipe(append, '1').pipe(append, '2');
      var replaced = original.without(original.get(by.task(append))[0]);
  ...
  describe('replace()', function () {
    it('should be able to replace with constructor', function (done) {
      ...
      pipeline = pipeline.replace(pipeline.get(by.task(append))[0]).with(append, '3');
    ...
    it('should be able to replace with a step object', function (done) {
      ...
      pipeline = pipeline.replace(pipeline.get(by.task(append))[0]).with({task: append, args: ['3']});
    ...
    it('should be able to replace with another mutapipe pipeline', function (done) {
      ...
      pipeline = pipeline.replace(pipeline.get(by.task(append))[0])
        .with(mutapipe().pipe(append, '4').pipe(append, '3'));

      write('abc').into(pipeline()).andExpect('abc432').then(done);
    });
    it('should leave the old pipeline intact', function (done) {
      var original = mutapipe().pipe(append, '1').pipe(append, '2');
      var replaced = original.replace(original.get(by.task(append))[0]).with(append, '3');
    ...
    it('should support nesting', function (done) {
      ...
      pipeline = pipeline.replace(pipeline.get(by.args('Replaced'))).with(
        mutapipe()
          .pipe(append, 'N1')
          .pipe(mutapipe()
            .pipe(append, 'N2'))
