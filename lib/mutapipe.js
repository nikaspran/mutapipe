...
function filter(steps, matcher) {
  return steps.filter(matcher);
}

function first(steps, matcher) {
  return (matcher ? steps.filter(matcher) : steps)[0];
}

function firstStepThatMatches(steps, selector) {
  if (isFunction(selector)) {
    selector = first(steps, selector);
  }
...
function PreviousResult(selector, method) {
  ...
  this.previousSteps = [];
  this.task = function () {
    var result = firstStepThatMatches(R.reverse(self.previousSteps), selector)._result; //TODO: validate it's a function
    return result[method].apply(result, arguments);
  };
}

module.exports = function mutapipe() {
  function createPipeline(steps, stepId) {
    function build() {
      //TODO: validate
      return combine.apply(null, steps.map(function (step) {
        step.args = step.args.map(function (arg) {
          return arg.build ? arg.build() : arg;
        });
    ...
    build.filter = filter.bind(build, steps);

    build.first = first.bind(build, steps);
